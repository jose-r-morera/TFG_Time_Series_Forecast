...
"""
Grid search for the parametesr and hyperparameters of LSTM with future covariates.
===========

Compares different architectures using grid search and valitdation loss.
...
x_train = data["train"]["air_temperature"]["past_variables"].astype(np.float32)
future_train = data["train"]["air_temperature"]["future_variables"].astype(np.float32)
y_train = data["train"]["air_temperature"]["y"].astype(np.float32)

x_val = data["test"]["air_temperature"]["past_variables"].astype(np.float32)
future_val = data["test"]["air_temperature"]["future_variables"].astype(np.float32)
y_val = data["test"]["air_temperature"]["y"].astype(np.float32)
...
def build_model(hp):
    ...
    # --- First LSTM layer ---
    units_1 = hp.Int('units_1', min_value=16, max_value=256, step=28)
    ...
    if use_second_lstm:
        ...
        if bidir_2:
            units_2 = hp.Int('units_2', min_value=16, max_value=256, step=28)
            past_layers = layers.Bidirectional(layers.LSTM(units_2, return_sequences=False))(past_layers)
        else:
            units_2 = hp.Int('units_2', min_value=16, max_value=512, step=28)
    ...
    if bidir_f:
        units_f = hp.Int('units_f', min_value=16, max_value=256, step=28)
        future_layers = layers.Bidirectional(
            layers.LSTM(units_f, name="lstm_future", return_sequences=False) 
        )(future_input)
    else:
        units_f = hp.Int('units_f', min_value=16, max_value=512, step=28)
